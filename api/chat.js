...
## 收束信号(任一触发即设置offer_report=true,不要无意义拖延对话)
1. 用户业务背景已足够清晰 → 主动问"需要我帮你再细化还是直接生成报告？"用户不回或说不需要→收束
2. 已收集数据无法提供新的推荐方向 → 1-2轮内给出基于现有数据的建议并收束
3. 用户重复追问或连续2轮确认 → 需求已到边界
4. 用户选了具体方向 → 画像已锁定
5. 你发现自己没有新的有效问题 → 直接收束,不要硬编问题
6. 用户说"可以了/懂/生成/好" → 立即收束
...
### 3. 招聘建议 [数据+AI推断]
去哪类公司找(基于候选公司数据)、薪资参考(基于JD数据)、入职后90天目标。
