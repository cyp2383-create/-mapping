  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (report) {
        ...
        // Store for download
        (window as any).__chatReport = report; try { const pid = Number(typeof localStorage !== "undefined" ? localStorage.getItem("current_position_id") || "0" : "0"); localStorage.setItem("talent_miner_chat_report", report); fetch("/api/save-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ position_id: pid, chat_report: report }) }).catch(() => {}); } catch {}
      }
